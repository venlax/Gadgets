...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        ...
        let res = Png {
            header: 
                value[0..8]
                    .try_into()
                    .map_err(|_| "Slice is not of length 8")?,
            chunks: {
                let mut chunks = Vec::new();
                let mut i = 8;
                while i < value.len() {
                    let chunk = Chunk::try_from(&value[i..])?;
    ...
    if res.header != Png::STANDARD_HEADER {
        dbg!(res.header);
        return Err(Box::new(DataError));
    }    
    Ok(res)
...
impl Png {
    ...
    pub fn remove_first_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        if let Some(pos) = self.chunks.iter().position(|chunk| chunk.chunk_type().bytes() == chunk_type.bytes()) {
